...
import { Plus, ChevronLeft, ChevronRight, RefreshCw, Calendar, Bell, CheckCircle2, Clock, Trash2, Pencil } from "@/lib/icons";
...
export default function Agenda() {
  ...
  const toggleConcluido = async (evento: Evento) => {
    await (supabase as any).from("agenda_eventos")
      .update({ concluido: !evento.concluido }).eq("id", evento.id);
    fetchEventos();
  };
  ...
  const eventosNoDia = (date: Date) =>
    eventos.filter(e => isSameDay(parseISO(e.data_inicio), date));

  const eventosHoje = useMemo(() =>
    eventos.filter(e => isToday(parseISO(e.data_inicio))), [eventos]);
            ...
            return (
              <div key={day.toString()}
                ...
                <div className="space-y-0.5">
                  {evs.slice(0, 3).map(e => (
                    <div key={e.id}
                      className={cn("text-[10px] px-1.5 py-0.5 rounded truncate font-medium", e.concluido && "line-through opacity-50")}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Eventos, lembretes e tarefas</p>
          </div>
        </div>
      ...
      {proximosEventos.length > 0 && (
        <div className="flex gap-3 overflow-x-auto pb-2">
          {proximosEventos.map(e => (
            <div key={e.id}
              onClick={() => { setSelectedDate(parseISO(e.data_inicio)); setView("dia"); }}
